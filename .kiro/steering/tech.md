...
- **Provider Abstraction**: `IFirewallProvider` interface with `ProviderRegistry` for multi-provider support (Vercel stable, Cloudflare and Fastly beta). `PROVIDER_TYPES` in `IFirewallProvider.ts` is the single source of truth for the closed set of known providers — see `.kiro/steering/adding-a-provider.md` for the current, short checklist of what adding one actually touches (deliberately restructured in August 2026 so this isn't a dozen-file change anymore).
...
- **TS assertion functions** (`asserts x is T`, e.g. `ValidationService.validateConfig`) can only be called through a variable with an _explicit_ type annotation: `const v: ValidationService = ValidationService.getInstance(); v.validateConfig(x)`. Calling `ValidationService.getInstance().validateConfig(x)` inline fails to compile (TS2775/TS2776) — this applies whenever calling an assertion-typed method off a freshly-returned instance rather than a pre-declared variable.
